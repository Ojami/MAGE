...
%             table).
% @13JUNE2024: 'qc' can be -2 to include all ethnicities available. 
% @13AUG2025: 'wes' is deprecated, since we use step 1 REGENIE, it should
%             be included even for WES analysis.

arguments
    opts.trait {mustBeText} = "" % if left empty, UKBtrait_toolbox will fetch traits
    opts.qc (1,1) double {mustBeMember(opts.qc, -2:6)} = 0 % see getQCEID function for more details
    ...
    opts.phenopath {mustBeFolder} = "D:\MATLAB.dependencies\MAGE\UKBB.Toolbox\UKB_PHENO"
...
if opts.wes
    opts.wes = false;
    fprintf("\t'wes' is deprecated (was set to false)\n")
end
...
% check traits
if opts.trait == ""
     % @15/10/2021: getPheno is more flexible than UKBtrait_toolbox
    % inputOpts.layer1 = 2;
    % inputOpts.layer2 = 1;
    % opts.trait = UKBtrait_toolbox(inputOpts);
