...
arguments
    ...
    opts.mountNetwork (1,1) logical = false % to check if network drives are mounted (only if ispc is true)
    opts.parseMsg (1,1) logical = true % to parse: strip->splitlines stdout
end
...
if opts.conda
    ...
    [fid, msg] = system(cmdpref + '"' + opts.dirwsl + bashname + '"');
    
    %@07NOV2024: temp fix for dx command
    if any(contains(string(msg), "command not found")), fid = 1; end
    ...
    if fid
        ...
        if fid
            [~, msg] = system(cmdpref + '/bin/bash -ilc "' + opts.dirwsl + bashname + '"');
        end
    end
end
