...
arguments
    ...
    %@16MAY2025: column names, required only when the summary statistic
    %format cannot be inferred automatically.
    opts.varid_col {mustBeText, mustBeVector} = string(missing) % variant id col, usually as chr:pos:ref:al format
    opts.snp_col {mustBeText, mustBeVector} = string(missing) % SNP column
    opts.ea_col {mustBeText, mustBeVector} = string(missing) % effect allele column
    opts.eaf_col {mustBeText, mustBeVector} = string(missing) % effect allele frequency column  
    opts.nea_col {mustBeText, mustBeVector} = string(missing) % other allele 
    opts.effect_col {mustBeText, mustBeVector} = string(missing) % effect column (beta)
    opts.p_col {mustBeText, mustBeVector} = string(missing)
    opts.n_col {mustBeText, mustBeVector} = string(missing) 
    opts.se_col {mustBeText, mustBeVector} = string(missing)
    opts.chr_col {mustBeText, mustBeVector} = string(missing)
    opts.pos_col {mustBeText, mustBeVector} = string(missing)
...
if opts.mat
    ...
    % @16MAY2025: added support for gz files 
    if strcmp(infileExt, ".gz")
        infile = gunzip(infile, pwd);
        infile = string(infile{1});
        [~, ~, infileExt] = fileparts(infile);
        infileChar = infile;
        opts.gz = true;
    else
...
opts.custom = false; % @16MAY2025: to handle custom formats
...
elseif opts.mat
    ...
    %@16MAY2025: try to infer variable names (e.g. taken from GWAS Catalog
    %harmonized data). 
    
    opts.legacy = false;
    opts.custom = true;
    
    % check that col names are provided and rename
    cc1 = ["varid_col", "snp_col", "ea_col", "eaf_col", "nea_col" , ...
        "effect_col", "p_col", "n_col", "se_col", "chr_col", "pos_col"];
    cc2 = ["varid", "snp", "a1", "a1freq", "a2", "beta", "p", "n", "se", ...
        "chr", "pos"];
    dict = dictionary(); % to keep col names for extraction
    for k = 1:numel(cc1)
        if ~ismissing(opts.(cc1(k)))
            dict(opts.(cc1(k))) = cc2(k);
        end
    end
    infile.SelectedVariableNames = dict.keys;

    % error('readGWASfile::unknownFormat:if file has a custom format, save it as a MAT file first!')
end

if ~opts.mat && ~opts.custom
    ...
    if opts.full % read extra columns 
        ...
        if any(contains(lower(header), ["af_allele2", "a1freq", "a1_freq", "maf", "a2freq", "eaf"]))
            annotateCols = header(contains(lower(header), ["af_allele2", "a1freq", "a1_freq", "maf", "a2freq", "eaf"]));
            infile.SelectedVariableNames = [infile.SelectedVariableNames, cellstr(annotateCols(1)).'];
        end
...
if opts.mat
    ...
    if opts.parallel
        ...
        if any(contains(infile.Properties.VariableNames, 'log10p')) % REGENIE
            infile.log10p = 10.^-infile.log10p;
            infile = renamevars(infile, "log10p", "p");
        end
...
% @16MAY2025: if .gz file delete temp file
if opts.gz
    delete(infileChar);
end
