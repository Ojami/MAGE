...
arguments
       ...
       opts.home {mustBeFolder} % must only contains unique BGEN files for each chr
       ...
       opts.tallout (1,1) logical = false % same as above, but tall inside 'getbulkgeno' function.
...
% if opts.tallout, opts.datatype = "double"; end 
...
%     % read bgenheader only if opts.eid is present (sample subsetting)
%     elseif ~isempty(opts.eid)
%          % under the assumption that samples are numeric, which is valid
%          % for UK Biobank samples. Also, sample block must exist,
%          % otherwise, samples variable will be empty and all samples will
%          % be read in this case
%          [opts.N, opts.samples] = bgenheader(inopts.bgenfile, ...
%              'sample', true, 'verbose', false);
% 
%          if any(ismissing(opts.samples))
%              opts.eid = [];
%          else
%              if isstring(opts.samples)
%                  if contains(opts.samples(1), "_")
%                      opts.samples = double(extractBefore(opts.samples, "_"));
%                  end
%              end
%              opts.samples = ismember(opts.samples, opts.eid);
%          end
...
if opts.merge
    ...
    if opts.tallout
        idx = tall(idx); idx2 = tall(idx2);
    end

    for i = 1:numel(geno.bed)
        
        if opts.tallout
            cz = gather(size(geno.bed{i}, 2));
            cchunk = unique([1:opts.chunk:cz, cz+1]);
            tmp = cell(numel(cchunk)-1, 1);
            for k = 1:numel(tmp)
                ccn = numel(cchunk(k):(cchunk(k+1)-1));
                tmp{k} = tall(zeros(numel(eid), ccn, opts.datatype));
            end
            tmp = horzcat(tmp{:});
        else
            tmp = nan(numel(eid), gather(size(geno.bed{i}, 2)), opts.datatype);
        end
...
if opts.dosage
    ...
    % @02/16/2022: commented due to the change made to bgenreader function
    %              previously, 'GP' flag with I_A equal to 1, was overrided
    %              by 'GT' flag. This had a negative impact on performance.

%     GTed = find(calls.I_A == 1);
%     PPed = setdiff(1:numel(calls.I_A), GTed);
%     GT = calls.bed(:, GTed.*2-1)+ calls.bed(:, GTed.*2);
%     if opts.model == "add"
%         bed(:, PPed) = calls.bed(:, PPed.*2-1)+ 2.*calls.bed(:, PPed.*2);
%     elseif opts.model == "dom"
%         bed(:, PPed) = calls.bed(:, PPed.*2-1)+ calls.bed(:, PPed.*2);
%         GT(GT > 1) = 1;
%     elseif opts.model == "rec"
%         bed(:, PPed) = calls.bed(:, PPed.*2);
%         GT(GT < 2) = 0;
%         GT(GT > 0) = 1;
%     end
%     bed(:, GTed) = GT;
