function res = fitSurvCox_legacy(preds, DV, covariates, opts2, opts)
...
r(2) = "suppressMessages(lapply(c" + ...
    ...
    '"fastcmprsk", "finalfit", "prodlim"), require, character.only = TRUE))';
r = [r, df'];
...
r(numel(r) + 1) = "fit_cif <- function(dat, genoCol, timeStop, entryCol, statusCol,";
r(numel(r) + 1) = "                    strataDF, times, causeLabel){";
r(numel(r) + 1) = "  form <- as.formula(";
r(numel(r) + 1) = "    sprintf(""Hist(%s, %s, entry = %s) ~ %s"",";
r(numel(r) + 1) = "            timeStop, statusCol, entryCol, genoCol))";
r(numel(r) + 1) = "  fit <- prodlim::prodlim(form, data = dat)";
r(numel(r) + 1) = "  pred <- predict(fit, newdata = strataDF, times = times,";
r(numel(r) + 1) = "                  cause = causeLabel, type = ""risk"")";
r(numel(r) + 1) = "  mat  <- do.call(cbind, lapply(pred, as.numeric))";
r(numel(r) + 1) = "  if (is.vector(mat)) mat <- matrix(mat, ncol = 1L)";
r(numel(r) + 1) = "  mat}";

% cif_boot_ci  (percentile bootstrap ribbons)
r(numel(r) + 1) = "cif_boot_ci <- function(df, genoCol,";
r(numel(r) + 1) = "                        timeStop, entryCol, statusCol,";
r(numel(r) + 1) = "                        causeLabel = ""event"",";
r(numel(r) + 1) = "                        times   = NULL,";
r(numel(r) + 1) = "                        nBoot   = 500,";
r(numel(r) + 1) = "                        seed    = 1,";
r(numel(r) + 1) = "                        parallel = FALSE,";
r(numel(r) + 1) = "                        nCores   = NULL) {";
r(numel(r) + 1) = "  set.seed(seed)";

% time grid
r(numel(r) + 1) = "  if (is.null(times)) {";
r(numel(r) + 1) = "    tmp   <- prodlim::prodlim(";
r(numel(r) + 1) = "               Hist(df[[timeStop]], df[[statusCol]],";
r(numel(r) + 1) = "                    entry = df[[entryCol]]) ~ 1)$time";
r(numel(r) + 1) = "    times <- sort(unique(unlist(tmp)))}";

% genotype strata
r(numel(r) + 1) = "  strataDF <- unique(df[, genoCol, drop = FALSE])";

% point estimate
r(numel(r) + 1) = "  pointMat <- fit_cif(df, genoCol,";
r(numel(r) + 1) = "                      timeStop, entryCol, statusCol,";
r(numel(r) + 1) = "                      strataDF, times, causeLabel)";

% bootstrap setup
r(numel(r) + 1) = "  bootFun <- function(idx) {";
r(numel(r) + 1) = "    fit_cif(df[idx, , drop = FALSE], genoCol,";
r(numel(r) + 1) = "            timeStop, entryCol, statusCol,";
r(numel(r) + 1) = "            strataDF, times, causeLabel)";
r(numel(r) + 1) = "  }";
r(numel(r) + 1) = "  idxMat <- matrix(";
r(numel(r) + 1) = "             sample.int(nrow(df), nrow(df) * nBoot, replace = TRUE),";
r(numel(r) + 1) = "             nrow = nBoot)";

% run bootstrap
r(numel(r) + 1) = "  if (parallel) {";
r(numel(r) + 1) = "    if (is.null(nCores))";
r(numel(r) + 1) = "      nCores <- max(1L, parallel::detectCores() - 1L)";
r(numel(r) + 1) = "    if (.Platform$OS.type == ""windows"")";
r(numel(r) + 1) = "      nCores <- min(nCores, 60L)";
r(numel(r) + 1) = "    clType <- if (.Platform$OS.type == ""windows"") ""PSOCK"" else ""FORK""";
r(numel(r) + 1) = "    cl <- parallel::makeCluster(nCores, type = clType)";
r(numel(r) + 1) = "    on.exit(try(parallel::stopCluster(cl), silent = TRUE))";
r(numel(r) + 1) = "    ## ensure required packages are on each worker (PSOCK needs this)";
r(numel(r) + 1) = "    parallel::clusterEvalQ(cl, { library(prodlim); library(data.table) })";
r(numel(r) + 1) = "    parallel::clusterExport(";
r(numel(r) + 1) = "      cl,";
r(numel(r) + 1) = "      c(""df"", ""genoCol"", ""timeStop"", ""entryCol"", ""statusCol"",";
r(numel(r) + 1) = "        ""strataDF"", ""times"", ""causeLabel"", ""fit_cif""),";
r(numel(r) + 1) = "      envir = environment())";
r(numel(r) + 1) = "    bootMats <- pbapply::pblapply(";
r(numel(r) + 1) = "                  X   = seq_len(nBoot),";
r(numel(r) + 1) = "                  FUN = function(b) bootFun(idxMat[b, ]),";
r(numel(r) + 1) = "                  cl  = cl)";
r(numel(r) + 1) = "  } else {";
r(numel(r) + 1) = "    bootMats <- pbapply::pblapply(";
r(numel(r) + 1) = "                  X   = seq_len(nBoot),";
...
% stack to 3-D array  nTime × nStrata × nBoot
r(numel(r) + 1) = "  arr <- array(unlist(bootMats),";
r(numel(r) + 1) = "               dim = c(length(times), ncol(pointMat), nBoot))";
r(numel(r) + 1) = "  lowMat  <- apply(arr, 1:2, quantile, .025, na.rm = TRUE)";
r(numel(r) + 1) = "  highMat <- apply(arr, 1:2, quantile, .975, na.rm = TRUE)";
r(numel(r) + 1) = "  kStrata    <- ncol(pointMat)";
r(numel(r) + 1) = "  labsStrata <- apply(strataDF, 1, paste, collapse = ""_"")";
r(numel(r) + 1) = "  data.table::data.table(";
r(numel(r) + 1) = "    time     = rep(times, kStrata),";
r(numel(r) + 1) = "    strata   = factor(rep(labsStrata, each = length(times)),";
r(numel(r) + 1) = "                      levels = labsStrata),";
r(numel(r) + 1) = "    cif      = as.numeric(pointMat),";
r(numel(r) + 1) = "    cif.low  = as.numeric(lowMat),";
r(numel(r) + 1) = "    cif.high = as.numeric(highMat)";
r(numel(r) + 1) = "  )";
r(numel(r) + 1) = "}";
...
if opts.timeOrigin == "TTE" && opts.LT
    % cuminc doesn't support LT
    % shift so entry ≥ 0
    r(numel(r) + 1) = "shift <- abs(min(df$tt0, na.rm = TRUE)) + 1e-6";
    r(numel(r) + 1) = "df$tt0.shift <- df$tt0 + shift";
    r(numel(r) + 1) = "df$tt.shift  <- df$tt  + shift";
    
    % Time grid
    r(numel(r) + 1) = "tt.grid <- seq(min(df$tt.shift), max(df$tt.shift), length.out = 200)";

    % bootstrap CIF with CI
    r(numel(r) + 1) = "dt <- cif_boot_ci(df, genoCol   = preds[k]," + ...
        "timeStop  = ""tt.shift"", entryCol  = ""tt0.shift""," + ...
        "statusCol = ""censor2"", times = tt.grid, nBoot = 250," + ...
        "seed = 42, parallel = TRUE, nCores = NULL)";
    
    % plot + save
    r(numel(r) + 1) = "      axlims <- range(dt$time - shift); axlims[1] <- 0";
    r(numel(r) + 1) = "      fig <- ggplot2::ggplot(dt,";
    r(numel(r) + 1) = "               ggplot2::aes(time - shift, cif,";
    r(numel(r) + 1) = "                            colour = strata, fill = strata)) +";
    r(numel(r) + 1) = "               ggplot2::geom_step(linewidth = .7) +";
    r(numel(r) + 1) = "               ggplot2::geom_ribbon(";
    r(numel(r) + 1) = "                   ggplot2::aes(ymin = cif.low, ymax = cif.high),";
    r(numel(r) + 1) = "                         alpha = .25, linewidth = 0) +";
    r(numel(r) + 1) = "               ggplot2::labs(x = ""Years"",";
    r(numel(r) + 1) = "                             y = ""Cumulative incidence"",";
    r(numel(r) + 1) = "                             colour = preds[k]) +";
    r(numel(r) + 1) = "               ggplot2::theme_light() +";
    r(numel(r) + 1) = "               ggplot2::scale_x_continuous(n.breaks = 6, limits = axlims)";
